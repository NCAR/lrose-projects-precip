...
radar='KAMX'; % KAMX, KBYX, KMLB, KTBW, SPOL, RCWF
...
countEdges=-70:0.5:70;
countsAllGPM=zeros(35,length(countEdges)-1);
countsAllGround=zeros(35,length(countEdges)-1);
...
for aa=1:length(listRadar)
    ...
    gpmOL(isnan(groundOL))=nan;
    groundOL(isnan(gpmOL))=nan;

    for jj=1:size(countsAllGPM,1)
        cGround=histcounts(groundOL(:,:,jj),countEdges);
        countsAllGround(jj,:)=countsAllGround(jj,:)+cGround;
        cGpm=histcounts(gpmOL(:,:,jj),countEdges);
        countsAllGPM(jj,:)=countsAllGPM(jj,:)+cGpm;
    end
    ...
    s2=subplot(2,2,2);
    surf(lonGPM,latGPM,plotGround,'EdgeColor','none');
    view(2);
    caxis([-5 70])
    colorbar('XTick',-5:3:70)
    title([radar,' DBZ (dBZ)']);
    ...
    print([figdir,radar,'_GPM_',num2str(dbzRange(2)),'dBZ_',infileGPM(1:15),'.png'],'-dpng','-r0');
end
...
print([figdir,radar,'_',num2str(dbzRange(2)),'dBZ_GPM.png'],'-dpng','-r0');

%% CFADS

countsAllGround(countsAllGround==0)=nan;
countsAllGPM(countsAllGPM==0)=nan;

maxCountGPM=prctile(countsAllGPM(:),99.9);
maxCountGround=prctile(countsAllGround(:),99.9);

maxAll=max(countsAllGPM,[],2,'omitnan');
[~,maxInd]=max(maxAll);

figure('Position',[200 500 800 1000],'DefaultAxesFontSize',12);

s1=subplot(5,1,1:2);
s1.Colormap=jet;
hold on
surf(countEdges(1:end-1),altGPM,countsAllGround,'edgecolor','none')
view(2);
xlim([0,40]);
ylim([0,20]);
caxis([0,maxCountGround]);
colorbar

ylabel('Altitude (km)');
title([radar,' (',num2str(sum(countsAllGround(:),'omitnan')),')']);
...
s2.Colormap=jet;
hold on
surf(countEdges(1:end-1),altGPM,countsAllGPM,'edgecolor','none')
view(2);
xlim([0,40]);
ylim([0,20]);
caxis([0,maxCountGPM]);
...
plot([0,11],[altGPM(maxInd),altGPM(maxInd)],'-k','LineWidth',1.5);

xlabel('Reflectivity (dBZ)');
ylabel('Altitude (km)');
title(['GPM (',num2str(sum(countsAllGPM(:),'omitnan')),')']);
...
s3=subplot(5,1,5);
hold on
plot(countEdges(1:end-1),countsAllGPM(maxInd,:),'-k','LineWidth',1.5);
plot(countEdges(1:end-1),countsAllGround(maxInd,:),'-b','LineWidth',1.5);
xlim([0,40]);
legend('GPM',radar)

xlabel('Reflectivity (dBZ)');
ylabel('Counts');
title(['Cross section at ',num2str(altGPM(maxInd)),' km']);
colorbar
...
s3pos=s3.Position;
s3.Position=[s3pos(1),s3pos(2)-0.05,s3pos(3),s3pos(4)];
set(colorbar, 'visible', 'off')

print([figdir,radar,'_GPM_CFAD.png'],'-dpng','-r0');