...
for ii=1:size(lambda_all,1)
 ...
 if nanstd(lambda_all) >= 5e-4    
    ...
    edges_on=828.180:.001:828.220;
    [value,edges]=histcounts(round(lambda_all,3),edges_on); % bin rounded wavelengths
    lambda_N = edges(value~=0)  % wavelength values
    %lambda_F = value(value~=0);  % frequency of occurance
    lambda_all_N = round(lambda_all,3);
    %figure(1234)
    %plot(lambda_all_N)
    %hold on
    %plot(lambda_all)
    %hold off
    lambda_N = lambda  % ADDED JUST FOR 28-JUL DATA 
